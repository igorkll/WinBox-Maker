...
");
        ...
        public async Task InstallToImg(string isoPath, string imgPath, bool useUefi=false)
        {
            ...
            string qemuParameters = $"-net none -accel whpx -drive file=\"{imgPath}\",format=raw -cdrom \"{isoPath}\" -boot d -m {winBoxConfig.img_install_ram} -smp {winBoxConfig.img_install_cpu} -D \"{getDebugFilePath("qemu-log")}\"";
...
reg add ""HKLM\SOFTWARE\Microsoft\Windows Embedded\KeyboardFilter"" /v BreakoutKeyScanCode /t REG_DWORD /d {winBoxConfig.keyboard_filter_BreakoutKeyScanCode} /f" + "\r\n";
        ...
        public string[] getStopServicesList()
        {
            ...
            string[] stopServicesList = Consts.defaultStopServicesList;
        ...
        public string[] getStopOrDeleteSchtasksList()
        {
            ...
            {
                stopOrDeleteSchtasks.AddRange(Consts.defaultStopOrDeleteSchtasks);
            }
        ...
        public async Task<bool> MakeModWim(Action<string> processName, Action<int> processValue, WindowsDescription newWindowsDescription, string newWimPath, string? imgExportPath, bool initViaVmMode = false)
        {
            ...
            if (!manual)
            {
                ...
                if (winBoxConfig.regtweak_overwrite_en == true)
                {
                    regTweakData = winBoxConfig.regtweak_overwrite;
                }
                else
                {
                    regTweakData = await File.ReadAllTextAsync(Program.mainTweakPath);
                }
...
{updateSystemSettingsAndFirstInit}";
                ...
                string firstInit = $@"echo FirstInit - setup services >> C:\WinboxResources\setup.log
{getServicesSetup()}" + "\r\n";
...
taskkill /F /FI ""COMMANDLINE like *run_first_setup_logo_hidden.vbs*""";
                ...
                if (!Program.isTweakEnabled(winBoxConfig, "Do not disable hotkeys by changing the layout"))
                {
                    ...
                    baseSetup += "\r\n";
                    baseSetup += $@"reg add ""HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Keyboard Layout"" /v ""Scancode Map"" /t REG_BINARY /d {winBoxConfig.sharpkeys_export} /f";
                }
