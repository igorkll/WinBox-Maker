...
namespace WinBox_Maker
{
    public partial class EditorForm : Form
    {
        ...
        {
            LockForm();
            string? name = await winBoxProject.SelectResourceAsync(UpdateProcessName, UpdateProcessValue, "Executable files (*.exe;*.bat;*.cmd;*.appx;*.msix)|*.exe;*.bat;*.cmd;*.appx;*.msix|All files (*.*)|*.*", Path.Combine(winBoxProject.resourcesDirectoryPath, "program"), true);
            if (name != null)
            {
